...
/**
 * @ Author   ：yangyunlong.
 * @ Date     ：Created in 16:29 2021/03/09
 * @Version ： 1.0
 */
public class PoiUtil {
    ...
    public static void createExcel2FilePath(String sheetName, String title, String filePath,
        ...
        PoiUtil poiExcelUtil = new PoiUtil();
        // 1.创建 Workbook
    ...
    private void writeExportDate(HSSFWorkbook hssfWorkbook, HSSFRow row, int cellIndex) {
        ...
        cellValueHandler(cell4ExportDateValue,
                DateUtil.dateToString2(new java.util.Date()));
    ...
    public static HSSFWorkbook createExcel2Export1(String sheetName, String title, String[] headers,
                                                  List<LinkedHashMap<String,Object>> dataList) throws Exception {
        ...
        poiExcelUtil.writeHeader(hssfWorkbook, hssfSheet, headers, title);
        // 4.写入内容
        poiExcelUtil.writeContent1(hssfWorkbook, hssfSheet, headers, dataList);

        return hssfWorkbook;
    }

    private void writeContent1(HSSFWorkbook hssfWorkbook, HSSFSheet hssfSheet,
                               String[] headers, List<LinkedHashMap<String,Object>> dataList) {
        // 单元格样式
        HSSFCellStyle cellStyle = createContentCellStyle(hssfWorkbook);
        ...
        for (int i = 2; i < dataList.size()+2; i++) {
            HSSFRow row6 = hssfSheet.createRow(i);
            List<String> stringList = stringlist.get(i - 2);
            for (int j = 0; j < stringList.size(); j++) {
                HSSFCell cell5 = row6.createCell(j);
                cell5.setCellStyle(cellStyle);
                cell5.setCellValue(stringList.get(j));
            }
        }
